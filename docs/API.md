...
Один URL, два ответа — по заголовку `Accept`:

* браузер (`Accept: text/html,...`) получает брендированную HTML-страницу
  (`static/subscription.html`) со статусом подписки, подбором клиента по ОС,
  кнопкой добавления подписки (deep-link для Hiddify/V2Box/v2rayNG/Shadowrocket —
  только там, где такая схема реально задокументирована и надёжна) и списком
  серверов по отдельности как fallback для клиентов, которые не принимают
  ссылку подписки целиком (например v2rayN на Windows);
* любой другой клиент (VLESS-приложение) получает raw-подписку как раньше —
  без изменений в этой ветке.

Иконки клиентов — реальные, взятые из открытых репозиториев соответствующих
проектов (`static/clients/*.png`), отданы через отдельный `StaticFiles` mount
на `/static/clients`.
