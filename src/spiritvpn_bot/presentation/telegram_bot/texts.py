...
# TODO: убрать после запуска сервиса оплаты — до тех пор это единственный
# способ выдать доступ без ручного вмешательства (см. config.py:
# friends_shared_code). Код в <code>...</code> — Telegram копирует его по тапу
# сам, без какого-либо кода с нашей стороны, поэтому нужен parse_mode=HTML на
# отправке этого сообщения (см. handle_start).
WELCOME_FREE_TRIAL_TEMPLATE = (
    "\n\n🎁 Сервис ещё в разработке, но вы можете получить бесплатный доступ на 30 дней. "
    "Отправьте боту в чат сообщение-код для получения доступа: <code>{code}</code>"
)
